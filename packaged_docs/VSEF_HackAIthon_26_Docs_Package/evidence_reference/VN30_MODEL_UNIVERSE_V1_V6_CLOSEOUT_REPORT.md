...
The final closeout verdict is negative for replacement claims. V1-V6 did not produce a confirmed replacement for the existing 61.61% absolute-direction classical champion. The strongest apparent model-universe wins were explained by final-only discovery, market-relative class imbalance, seed instability, or failure to transfer after validation-governed relock. QML is excluded from the HackAIthon MVP scope.
...
| V3 skipped families | Enable statistical, deep sequence, and ensemble families | BiLSTM market_relative_vn30 h40 reached validation 64.06%, final 72.50%, labeled future_blind_required. | Promising diagnostic, not claimable. |
...
V3 extended coverage to statistical, deep sequence, and ensemble diagnostics. Statistical and deep/sequence families produced validation improvements in some settings, but validation-to-final transfer and repaired metrics did not support a replacement claim.
...
V5 completed the demotion. The BiLSTM's 72.50% raw final result was mostly class-imbalance driven, because the final majority baseline was 79.69% and repaired final metrics were weak. The row is not a replacement for the 61.61% absolute-direction champion and not a live or trading result.
...
5. Keep absolute_direction champion comparisons separate from market_relative_vn30 comparisons.
